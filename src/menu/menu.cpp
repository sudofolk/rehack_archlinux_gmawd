...
void Menu::onSwapWindow(SDL_Window* window) {
    if (!initialised) {
        std::cout << "Initializing SDL_GL_SwapWindow..." << std::endl;
        ...
        ImFontConfig config;
        ImGui::GetIO().Fonts->AddFontFromMemoryCompressedTTF(Roboto_compressed_data, Roboto_compressed_size, 12.f);
        //ImGui::GetIO().Fonts->AddFontFromFileTTF("/usr/share/fonts/adobe-source-code-pro/SourceCodePro-Medium.otf", 14.0f, &config);
        initialised = true;
    }
...
void Menu::drawOverlay(ImDrawList* drawList){
    if(CONFIGBOOL("Visuals>Enabled") && Globals::LocalPlayer){
        ESP::Render(drawList);
    }
...
void Menu::drawMenu() {
    ...
    ImGui::Begin("gmod_re", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse);
    ImGui::Separator();
    ImGui::Separator();
    ImGui::Checkbox("ESP?", &CONFIGBOOL("Visuals>Enabled"));
    ImGui::End();
}