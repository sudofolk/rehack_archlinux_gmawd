...
public:
	...
	/*/0*/	virtual void* unk() = 0;
	/*1*/	virtual void* IHandleEntityD() = 0;
	/*2*/	virtual void* SetRefEHandle(void*) = 0;
	/*3*/	virtual void* GetRefEHandle(void)const = 0;
	/*4*/	virtual CCollisionProperty* GetCollideable(void) = 0;
	/*5*/	virtual IClientNetworkable* GetClientNetworkable(void) = 0;
	/*6*/	virtual IClientRenderable* GetClientRenderable(void) = 0;
	...
	/*11*/	virtual Vector& GetAbsOrigin(void)const = 0;
	/*12*/	virtual QAngle& GetAbsAngles(void)const = 0;
	/*13*/	virtual void* GetMouth(void) = 0;
	/*14*/	virtual void* GetSoundSpatialization(SpatializationInfo_t&) = 0;
	/*15*/	virtual ClientClass* GetClientClass(void) = 0;
	/*1444444*/	virtual void* YouForgotToImplementOrDeclareClientClass(void) = 0;
	...
	/*233*/	virtual void PushEntity(void) = 0; // HERE
	...
	int getFlags() {
		return *reinterpret_cast< int* >( uintptr_t( this ) + 0x430 ); 
	}
	...
	CCollisionProperty* collideable(){
		return (reinterpret_cast<CCollisionProperty*>(uintptr_t(this) + 0x210));
	}

	int health(){
		return *reinterpret_cast<int*>(uintptr_t(this) + 0xB8);
	}
	...
	Vector origin(){
		return *(reinterpret_cast<Vector*>(uintptr_t(this) + 0x418));
	}

	Vector mins(){
		Vector* minz = reinterpret_cast<Vector*>(uintptr_t(this->collideable()) + 0x28 );
	...
	Vector maxs(){
		Vector* maxz = reinterpret_cast<Vector*>(uintptr_t(this->collideable()) + 0x34);
	...
	bool dormant(){
		bool* dormant = reinterpret_cast<bool*>(uintptr_t(this) + 0x78);
	...
	Vector getVelocity() {
		return *(Vector*)((uintptr_t)this + 0x1C0);
	}

	Vector eyepos(){ 
		return origin() + *reinterpret_cast<Vector*>((uintptr_t)this + 0x38DC );
	}

	C_BaseCombatWeapon* getweapon(){
		C_BaseCombatWeapon* wep = reinterpret_cast<C_BaseCombatWeapon*>((uintptr_t)this + 0x2934 );
	...
	Vector GetBonePosition(int bone, int curtime) {
		matrix3x4_t boneMatrixes[128];
		if (this->GetClientRenderable()->SetupBones(boneMatrixes, 128, 0x100, curtime)) {
	...
	inline void push_entity(){
		using push_entity_t = void*(__thiscall*)(void*);
		return method<push_entity_t>(232, this)(this);
	}
	...
	* Double click it.
	* There's your offset.
	* For exemple, in https://2i.imgur.com/88fYuYY.png this case, the offset will be 0x01F4 AND NOT 0x010F4 !
	*/
