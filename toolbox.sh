...
gdb="gdb"
libname="resources.so" # Pretend to be gamemode, change this to another lib that may be in /maps (if already using real gamemode, I'd suggest using libMangoHud.so)
csgo_pid=$(pidof gmod)
...
function load {
    ...
    sudo cp build/libgmod_re.so /usr/lib/$libname
    gdbOut=$(
      $gdb -n -q -batch \
